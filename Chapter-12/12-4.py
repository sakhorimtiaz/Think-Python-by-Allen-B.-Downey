...
#but the thing is my code can find out all possibilities, i got three in the output. But the solved code 
#can find only one. Check the output after each code
...
def make_word_dict():
    ...
    with open('C:\\Users\\THINKPAD\\Downloads\\words.txt') as file:
        for line in file:
            word=line.strip()
            d[word]=None
        """None is used as a placeholder value to signify that the dictionary's primary function is to store 
        the presence of words,not their values. It consumes minimal memory.
        "a", "I", and "" are given values of themselves to make them easily identifiable as base cases or 
        special cases in your code. During the reducibility checks, you may want to quickly verify that 
        "a", "i", and "" are base cases or special cases that need to be treated differently.
        The empty string "" is used as a base case for recursive functions, while a space " " 
        would not serve the same purpose and is not used here.In the context of word reducibility, 
        you might reduce a word down to nothing, and the empty string would represent that final state. 
        This base case is crucial for recursive functions, as it often serves as the simplest, 
        irreducible form of a word."""
        for letters in ["a","i",""]:
            d[letters]=letters
        return d
...
def print_all_trails(word_dict):
    #total reducible words
    words=all_reducible(word_dict)
    total_reducible_words=len(words)
    print(total_reducible_words)

    #longets reducible word length
    words_sorted_by_length=sorted(words,key=len,reverse=True)
    longest_words_length=len(words_sorted_by_length[0])

    #longest trail, we dont know yet, the number of longest words/word
    for word in words_sorted_by_length:
        if len(word)==longest_words_length:
            print_trail(word)
        break
    """The moment the loop encounters a word that is shorter than the longest reducible word, 
    the loop stops. This is because the words have already been sorted by length, 
    so any subsequent words will be even shorter 
    and not of interest for printing as the "longest reducible words."""
